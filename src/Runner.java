...
public class Runner {
    public static void main(String[] args) {
        ...
        try {
            ...
            List<ExRate> exchangeRateList=new ArrayList<ExRate>();
            for (int i=0;i<nodeList.getLength();i++){
                exchangeRateList.add(getExchange(nodeList.item(i)));
            }
            for (ExRate exchangeRate:exchangeRateList){
    ...
    private static ExRate getExchange(Node item) {
        ExRate exchangeRate=new ExRate();
